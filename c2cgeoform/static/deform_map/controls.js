 ...
 */
c2cgeoform.InteractionControl = function (
  className,
  tooltipLabel,
  source,
  callback
...
c2cgeoform.DrawPointControl = function (source, onDrawCallback, tooltip) {
  c2cgeoform.InteractionControl.call(
    this,
    "draw-point",
    tooltip,
    source,
...
c2cgeoform.DrawLineControl.prototype.createInteractions = function (source) {
  var drawInteraction = new ol.interaction.Draw({
    ...
    style: c2cgeoform.measureStyle(
      c2cgeoform.createDefaultEditingStyles(),
      "LineString"
    ),
  });
  drawInteraction.on("drawend", this.callback);
...
c2cgeoform.ModifyControl.prototype.createInteractions = function (source) {
  ...
  styles[c2cgeoform.GeometryType.POLYGON] = styles[
    c2cgeoform.GeometryType.POLYGON
  ].concat(styles[c2cgeoform.GeometryType.LINE_STRING]);
  styles[c2cgeoform.GeometryType.GEOMETRY_COLLECTION] = styles[
    c2cgeoform.GeometryType.GEOMETRY_COLLECTION
  ].concat(styles[c2cgeoform.GeometryType.LINE_STRING]);
...
c2cgeoform.zoomToGeometry_ = function (map, geometry, zoomForGeometry) {
  ...
  zoomForGeometry =
    zoomForGeometry === undefined
      ? map.get("zoomForGeometry")
      : zoomForGeometry;

  map.getView().fit(geometry, map.getSize(), { maxZoom: zoomForGeometry });
 ...
 */
c2cgeoform.addFeature = function (
  map,
  source,
  geoJson,
  zoomForGeometry,
...
c2cgeoform.initializeToolbar = function (map, source, options) {
  ...
  var onChangeCallback = function () {
    ...
    if (source.getFeatures().length > 0) {
      if (!options.controlsDefinition.isMultiGeometry) {
        ...
        geometry = c2cgeoform.makeGeometryCollection_(
          source.getFeatures(),
          options.controlsDefinition
        );
      }
    }
    var geoJson = "";
  ...
  if (options.controlsDefinition.point) {
    toolbar.addControl(
      new c2cgeoform.DrawPointControl(
        source,
        onDrawCallback,
        options.controlsDefinition.drawPointTooltip
      )
    );
  }
  if (options.controlsDefinition.line) {
    toolbar.addControl(
      new c2cgeoform.DrawLineControl(
        source,
        onDrawCallback,
        options.controlsDefinition.drawLineTooltip
      )
    );
  }
  if (options.controlsDefinition.polygon) {
    toolbar.addControl(
      new c2cgeoform.DrawPolygonControl(
        source,
        onDrawCallback,
        options.controlsDefinition.drawPolygonTooltip
      )
    );
  }
  toolbar.addControl(
    new c2cgeoform.ModifyControl(
      source,
      onChangeCallback,
      options.controlsDefinition.modifyTooltip
    )
  );
  toolbar.addControl(
    new c2cgeoform.ClearFeaturesControl(
      source,
      onChangeCallback,
      options.controlsDefinition.clearTooltip
    )
  );
...
c2cgeoform.createDefaultEditingStyles = function () {
  ...
  styles[c2cgeoform.GeometryType.CIRCLE] = styles[
    c2cgeoform.GeometryType.POLYGON
  ].concat(styles[c2cgeoform.GeometryType.LINE_STRING]);
  ...
  var create_linestring_style = function (linestring) {
    ...
    linestring.getCoordinates().forEach(function (coordinates, index) {
      ...
      var geom = new ol.geom.LineString([
        linestring.getCoordinates()[index],
    ...
    if (for_type == undefined || type == for_type) {
      if (type == "Polygon") {
        feature
          .getGeometry()
          .getLinearRings()
          .forEach(function (linear_ring) {
            measure_style = measure_style.concat(
              create_linestring_style(linear_ring)
            );
          });
      }
      ...
      if (type == "LineString") {
        measure_style = measure_style.concat(
          create_linestring_style(feature.getGeometry())
        );
      }
    }
