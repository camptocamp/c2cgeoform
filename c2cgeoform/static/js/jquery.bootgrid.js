...
(function ($, window, undefined) {
    ...
    firstHeadRow.children().each(function () {
      var $this = $(this),
        ...
        column = {
          ...
          converter:
            that.options.converters[data.converter || data.type] ||
          ...
          order:
            !sorted && (data.order === "asc" || data.order === "desc")
              ? data.order
              : null,
    ...
    function containsPhrase(row) {
      var column,
        searchPattern = new RegExp(
          that.searchPhrase,
          that.options.caseSensitive ? "g" : "gi"
        );
    ...
    if (this.options.ajax) {
      ...
      var rows =
          this.searchPhrase.length > 0
            ? this.rows.where(containsPhrase)
            : this.rows,
        total = rows.length;
      if (this.rowCount !== -1) {
  ...
  function loadRows() {
    if (!this.options.ajax) {
      ...
      this.totalPages =
        this.rowCount === -1 ? 1 : Math.ceil(this.total / this.rowCount);
  ...
  function prepareTable() {
    ...
    if (this.options.navigation & 1) {
      this.header = $(
        tpl.header.resolve(
          getParams.call(this, { id: this.element._bgId() + "-header" })
        )
      );
    ...
    if (this.options.navigation & 2) {
      this.footer = $(
        tpl.footer.resolve(
          getParams.call(this, { id: this.element._bgId() + "-footer" })
        )
      );
  ...
  function renderActions() {
    if (this.options.navigation !== 0) {
      ...
      if (headerActions.length + footerActions.length > 0) {
        ...
        // Refresh Button
        if (this.options.ajax) {
          var refreshIcon = tpl.icon.resolve(
              getParams.call(this, { iconCss: css.iconRefresh })
            ),
            refresh = $(
              tpl.actionButton.resolve(
                getParams.call(this, {
                  content: refreshIcon,
    ...
    if (this.options.columnSelection && this.columns.length > 1) {
      var that = this,
        ...
        icon = tpl.icon.resolve(
          getParams.call(this, { iconCss: css.iconColumns })
        ),
        dropDown = $(
          tpl.actionDropDown.resolve(getParams.call(this, { content: icon }))
        ),
      ...
      $.each(this.columns, function (i, column) {
        var item = $(
          tpl.actionDropDownCheckboxItem.resolve(
            getParams.call(that, {
              name: column.id,
              label: column.text,
          ...
          if (!checkbox.prop("disabled")) {
            ...
            $this
              .parents(itemsSelector)
              .find(selector + ":has(" + checkboxSelector + ":checked)")
          ...
          infos = $(
            this.options.templates.infos.resolve(
              getParams.call(this, {
                  ...
                  this.total === 0 || end === -1 || end > this.total
                    ? this.total
                    : end,
  ...
  function renderPagination() {
    if (this.options.navigation !== 0) {
      var selector = getCssSelector(this.options.css.pagination),
        headerPagination = this.header
          .find(selector)
          ._bgShowAria(this.rowCount !== -1),
        footerPagination = this.footer
          .find(selector)
          ._bgShowAria(this.rowCount !== -1);

      if (
        this.rowCount !== -1 &&
        headerPagination.length + footerPagination.length > 0
      ) {
        var tpl = this.options.templates,
          current = this.current,
          ...
          startWith =
            offsetRight >= this.options.padding
        ...
        renderPaginationItem
          .call(this, pagination, "first", "&laquo;", "first")
          ._bgEnableAria(current > 1);
        renderPaginationItem
          .call(this, pagination, "prev", "&lt;", "prev")
          ._bgEnableAria(current > 1);

        for (var i = 0; i < count; i++) {
          var pos = i + startWith;
          renderPaginationItem
            .call(this, pagination, pos, pos, "page-" + pos)
            ._bgEnableAria()
        ...
        if (count === 0) {
          renderPaginationItem
            .call(this, pagination, 1, 1, "page-" + 1)
            ._bgEnableAria(false)
            ._bgSelectAria();
        }

        renderPaginationItem
          .call(this, pagination, "next", "&gt;", "next")
          ._bgEnableAria(totalPages > current);
        renderPaginationItem
          .call(this, pagination, "last", "&raquo;", "last")
          ._bgEnableAria(totalPages > current);
  ...
  function renderPaginationItem(list, uri, text, markerCss) {
    var that = this,
      ...
      item = $(tpl.paginationItem.resolve(values)).on(
        "click" + namespace,
  ...
  function renderRowCountSelection(actions) {
    ...
    if ($.isArray(rowCountList)) {
      var css = this.options.css,
        tpl = this.options.templates,
        dropDown = $(
          tpl.actionDropDown.resolve(
            getParams.call(this, { content: this.rowCount })
          )
        ),
      ...
      $.each(rowCountList, function (index, value) {
        var item = $(
          tpl.actionDropDownItem.resolve(
            getParams.call(that, { text: getText(value), uri: "#" + value })
          )
        )
          ...
          .on("click" + namespace, menuItemSelector, function (e) {
            ...
            if (newRowCount !== that.rowCount) {
              ...
              $this
                .parents(menuItemsSelector)
                .children()
                .each(function () {
                  var $item = $(this),
                    currentRowCount = +$item
              ...
              $this
                .parents(menuSelector)
                .find(menuTextSelector)
                .text(getText(newRowCount));
              loadData.call(that);
            }
          });
  ...
  function renderRows(rows) {
    if (rows.length > 0) {
      ...
      $.each(rows, function (i, row) {
        ...
        rowAttr =
          ' data-row-id="' +
          (that.identifier == null ? i : row[that.identifier]) +
          '"';
        rowCss = "";

        if (selection) {
          var selected =
              $.inArray(row[that.identifier], that.selectedRows) !== -1,
            selectBox = tpl.select.resolve(
              getParams.call(that, {
                type: "checkbox",
          ...
          cells += tpl.cell.resolve(
            getParams.call(that, { content: selectBox, css: css.selectCell })
          );
        ...
        $.each(that.columns, function (j, column) {
          if (column.visible) {
            var value = $.isFunction(column.formatter)
              ...
              cssClass =
                column.cssClass.length > 0 ? " " + column.cssClass : "";
            cells += tpl.cell.resolve(
              getParams.call(that, {
        ...
        html += tpl.row.resolve(
          getParams.call(that, { attr: rowAttr, cells: cells })
        );
      });

      // sets or clears multi selectbox state
      that.element
        .find("thead " + getCssSelector(that.options.css.selectBox))
  ...
  function registerRowEvents(tbody) {
    ...
    if (selection) {
      tbody
        .off("click" + namespace, selectBoxSelector)
  ...
  function renderTableHeader() {
    ...
    if (selection) {
      var selectBox = this.options.multiSelect
        ? tpl.select.resolve(
            getParams.call(that, { type: "checkbox", value: "all" })
          )
        : "";
      html += tpl.rawHeaderCell.resolve(
        getParams.call(that, { content: selectBox, css: css.selectCell })
      );
    }
    ...
    $.each(this.columns, function (index, column) {
      if (column.visible) {
        var sortOrder = that.sort[column.id],
          ...
          cssClass =
            column.headerCssClass.length > 0 ? " " + column.headerCssClass : "";
        html += tpl.headerCell.resolve(
          getParams.call(that, {
            column: column,
            icon: icon,
    ...
    if (sorting) {
      ...
      headerRow
        .off("click" + namespace, sortingSelector)
        .on("click" + namespace, sortingSelector, function (e) {
          e.preventDefault();
          var $this = $(this),
            columnId =
              $this.data("column-id") ||
              $this.parents("th").first().data("column-id"),
  ...
  function showLoading() {
    var tpl = this.options.templates,
      ...
      padding =
        this.element.height() - thead.height() - (firstCell.height() + 20),
      count = this.columns.where(isVisible).length;
  ...
  Grid.prototype.select = function (rowIds) {
    if (this.options.selection && this.identifier != null) {
      ...
      while (
        rowIds.length > 0 &&
        !(!this.options.multiSelect && selectedRows.length === 1)
      ) {
        id = rowIds.pop();
      ...
      if (selectedRows.length > 0) {
        var selectBoxSelector = getCssSelector(this.options.css.selectBox),
          selectMultiSelectBox =
            this.selectedRows.length >= this.currentRows.length;

        i = 0;
        while (
          !this.options.keepSelection &&
          selectMultiSelectBox &&
          i < this.currentRows.length
        ) {
          selectMultiSelectBox =
            $.inArray(
              this.currentRows[i++][this.identifier],
              this.selectedRows
            ) !== -1;
        }
        this.element
          .find("thead " + selectBoxSelector)
          .prop("checked", selectMultiSelectBox);

        if (!this.options.multiSelect) {
          this.element
            .find("tbody > tr " + selectBoxSelector + ":checked")
            .trigger("click" + namespace);
        }

        for (i = 0; i < this.selectedRows.length; i++) {
          this.element
            .find('tbody > tr[data-row-id="' + this.selectedRows[i] + '"]')
          ...
          this.element
            .find(
              'tbody > tr[data-row-id="' +
                deselectedRows[i][this.identifier] +
                '"]'
            )
